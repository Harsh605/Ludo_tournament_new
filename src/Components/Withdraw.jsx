...
function WithdrawManager() {
    ...
    const fetchAdmindata = async () => {
        try {
            ...
            const response = await axios.get(`${baseURL}temp/withdraw/all/all/all`, {
            ...
            <tr role="row" key={index}>
                ...
                <td>
                    <div style={{ width: '17pc'}}>
                        {
                            (data?.status != "SUCCESS" && data?.status != "FAILED" && data?.status != "reject") ? <button className="mr-1 btn btn-sm btn-danger" onClick={() => withdrowFail(data?.txn_id)}>Reject By Admin</button> : ''
                        }

                        {
                            (data?.status != "SUCCESS" && data?.status != "FAILED" && data?.status != "reject") ? <button className="ml-1 btn btn-sm btn-warning" onClick={() => withdrowPass(data?.txn_id)}>Approve By Admin</button> : ''
                        }
                    </div>
                </td>
            </tr>
    ...
    return (
        <> <div className='fade-in'>
            <div style={{ paddingLeft: '2rem', marginTop: '4rem', paddingBottom: '2rem', borderBottom: '1px solid white' }}>
                <h3 style={{ color: 'white' }}>Withdraw Manager</h3>
            </div>
                                                ...
                                                <table className="table text-center dataTable no-footer dtr-inline" id="table_id" role="grid" aria-describedby="table_id_info" style={{}}>
                                                    <thead className="text-capitalize">
                                                        ...
                                                        <tr role="row">
                                                            ...
                                                            <th className="sorting" tabIndex={0} aria-controls="table_id" rowSpan={1} colSpan={1} style={{ width: 105 }} aria-label="To User: activate to sort column ascending">Upi I'd</th>
