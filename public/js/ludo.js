...
socket.on("connect", function () {
  ...
  socket.on("is-it-your-chance", function (data) {
    ...
    if (data === myid) {
      ...
      // Set a timeout for 20 seconds (adjust if needed)
  ...
  socket.on("Thrown-dice", async function (data) {
    ...
    const piece = PLAYERS[data.id].myPieces[data.pid];
    const { isFourPiceReachedHome, isPiceReachedHome, isKilled } = await piece.update(data.num);
    
    if (isFourPiceReachedHome) {
    ...
    // Determine the next player based on whether a piece was killed or not
  const nextPlayerId = (data.num === 6 || isKilled || isPiceReachedHome ) ? data.id : chanceRotation(data.id, data.num);
